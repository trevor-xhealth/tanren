...
/** Recreate the workspace with exactly these files, and reset the runner user's mise
 * state so each case starts from a fresh-runner posture (the global config + the
 * Tanren-provisioned marker are runner-scoped and would otherwise leak between cases). */
async function seedWorkspace(files: Readonly<Record<string, string>>): Promise<void> {
  ...
  const result = await ssh.run(target, {
    command: [
      ...
      'rm -f "$HOME/.config/mise/config.toml" "$HOME/.tanren-toolchain-provisioned"',
      ...writes,
    ].join("\n"),
  });

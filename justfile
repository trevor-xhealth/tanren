...
# BUDGET-SAFETY negative control for the codex→OpenRouter cost-attribution deadlock
# (docs/_design/openrouter-cost-attribution.md). Halves B/C need real Postgres: the
# REAL CostRecorder writes the run's OWN NULL-cost per_token rows, the REAL
# PgBudgetGate then fails CLOSED on `unpriced_spend` and RAISING THE CEILING DOES
# NOT CLEAR IT (the self-deadlock), and the run-setup preflight refuses the run up
# front instead. Half A (the real `codex exec --json` bytes) runs unconditionally in
# `just test`; without this recipe the six DB-gated tests executed in NO gate.
smoke-rls-openrouter-cost-attribution:
  DATABASE_URL="${DATABASE_URL:-postgres://tanren:tanren@localhost:5432/tanren}" TANREN_RLS_DB_TEST=1 corepack pnpm exec vitest run --no-file-parallelism services/orchestrator/tests/openRouterCostAttribution.integration.test.ts
...
# mq-10: the PRODUCTION PgAutonomousRepairRouter over the isolated `tanren_app` role.
# Proves the three routes end-to-end (repair / respec / blocked), that a re-spec
# INHERITS its stuck parent's SpecMode — pinned across all three modes, including
# `from_scratch` whose value is unchanged, so "inherit" is proven to be inheritance
# rather than a hardcode that happens to agree — and that a cross-org read sees ZERO
# rows. All seven tests are TANREN_RLS_DB_TEST-gated and ran in NO gate before this.
smoke-rls-merge-repair-routes:
  DATABASE_URL="${DATABASE_URL:-postgres://tanren:tanren@localhost:5432/tanren}" TANREN_RLS_DB_TEST=1 corepack pnpm exec vitest run --no-file-parallelism services/orchestrator/tests/mergeRepairRoutes.rls.integration.test.ts
